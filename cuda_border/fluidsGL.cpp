...
cData* v = new cData[DS];
cData* v_prev = new cData[DS];
...
extern "C" void diffuse(int dx, int dy, int n, int bx, int by, cData* v, cData* v_prev, float visc, float dt);
extern "C" void addforce(int dx, int dy, int n, cData* v,int spx, int spy, float dfx, float dfy);
extern "C" void project(int dx, int dy, int n, cData* v, cData* v_prev);
extern "C" void advect(int dx, int dy, int n, int bx,int by, cData* v, cData* v_prev, float dt);
extern "C" void advectParticles(GLuint vbo, cData* v, int dx, int dy, float dt);

void swap(cData* &u, cData* &v){
	cData* temp = u;
	u = v;
	v = temp;
}
...
void simulateFluids(void)
{
    // simulate fluid
	swap(v_prev, v); 
	diffuse(DIM,DIM,N,1,2, v, v_prev,VIS, DT);
	//project(DIM,DIM,N,v, v_prev);
	swap(v_prev, v);
	//advect(DIM,DIM,N,1,2, v, v_prev, DT);
	//project(DIM,DIM,N,v,v_prev);
    //advectParticles(vbo, v, DIM, DIM, DT);
}
...
void autoTest(char **argv)
{
    ...
    for (int count=0; count<g_iFrameToCompare; count++)
    {
        ...
        if (ref_file)
        {
            ...
            int spx = nx-FR;
			// c'est quoi FR ny nx ?
            addforce(DIM,DIM,N,v,spx,spy, FORCE*DT*fx,FORCE*DT*fy);
            lastx = x;
            lasty = y;
        }
    }
